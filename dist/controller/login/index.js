...
let loginUser = (0, tryCatch_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let user = yield model_1.Users.findOne({
        $or: [search, { email: username }],
    }).select("-__v -createdAt -updatedAt");
    if (user) {
        // <<======= Checking user account blocked =======>>
        const blockedUser = yield model_1.LoginAttempt.findOne({ username: user === null || user === void 0 ? void 0 : user.username });
        if (blockedUser) {
            ...
            yield new model_1.LoginAttempt({ ip: req.ip, username: username }).save();
        }
        // <<======= Validate Password =======>>
        ...
        if (pass) {
            ...
            if (blockedUser) {
                ...
                const blockedUser = new model_1.LoginAttempt({
                    ip: req.ip,
                    username: username,
                }).save();
            }
